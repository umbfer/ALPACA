...
for(alpha in alphaValues) {
    ...
    for (file in files) {
        ...
        dirname <- sprintf( "PointPlot-a=%s", substr(alphastr, 3, 5))
...
#			stop("break")
			...
			sp2 <- ggplot( data, aes(x = len, y = power, shape=factor(k), color=factor(k))) + theme_bw() + # theme_classic() +
				# ggtitle( title) + 
				scale_y_continuous(name = "Power Statistics", limits = c( threshold-0.1, 1)) +
			...
			sp2 <- sp2 + scale_color_manual(name = "", labels = lbls, breaks = levs, values = clrs ) +
			scale_shape_manual(name = "", labels = lbls, values = c( 1, 2, 3, 4)) # guides(shape = FALSE) # remove the legend for shapes
